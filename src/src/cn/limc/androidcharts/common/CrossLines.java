...
//            String textToDraw = inChart.calcAxisXGraduate();
            //xx create
            String textToDraw = inChart.xxCalcAxisXGraduate();
            ...
            PointF boxHS = new PointF(inChart.getTouchPoint().x - textWidth / 2.0f, inChart.getHeight() - fontSize -2  );
            PointF boxHE = new PointF(inChart.getTouchPoint().x + textWidth / 2.0f, inChart.getHeight() - 2 );
        ...
        if (displayCrossXOnTouch && displayCrossYOnTouch){
            {
                ...
                canvas.drawCircle( touchPoint.x, touchPoint.y, circleRadius, paint);
            }
            {
                int circleRadius      = 14;
                Paint paint = new Paint();
...
//                new float[] { 6, 3, 6, 3 }, 1));
        ...
        if (displayCrossYDegreeOnTouch) {
            ...
            if (this.inChart.getAxisYPosition() == Axis.AXIS_Y_POSITION_RIGHT){
                boxHS = new PointF(inChart.getWidth() + inChart.getBorderWidth() - inChart.getDataQuadrantPaddingRight(), inChart.getTouchPoint().y - fontSize / 2.f - 4);
                boxHE = new PointF(inChart.getWidth() - inChart.getBorderWidth(), inChart.getTouchPoint().y + fontSize / 2.f + 4);

                // draw a rectangle
                canvas.drawRect(boxHS.x, boxHS.y, boxHE.x, boxHE.y, mPaintBox);
                // draw text
                canvas.drawText(textToDraw, boxHS.x + inChart.getDataQuadrantPaddingRight()-textWidth, boxHS.y + fontSize, mPaintBoxText);
            }else{
                boxHS = new PointF(inChart.getBorderWidth(), inChart.getTouchPoint().y - fontSize / 2.f - 4);
                boxHE = new PointF(inChart.getBorderWidth() + textWidth, inChart.getTouchPoint().y + fontSize / 2.f + 4);
